...
function renderMarkdown(entries, gallery) {
  ...
  const topPartialKeys = new Set(topPartial.map((entry) => `${entry.kind}:${entry.name}`));
  const reviewedSmallSlices = [...partial]
    .filter((entry) => entry.localSourceReviewed === "yes" && !topPartialKeys.has(`${entry.kind}:${entry.name}`))
