...
// PGFPlots builds `every axis legend` as a TikZ matrix. For inline amsmath
// matrices, the observed line-sample-to-label gap is 2.57pt; the formula box
// itself now carries the matrix and delimiter advance and needs no extra cell
// allowance.
const PGFPLOTS_LEGEND_MATRIX_TEXT_GAP = 2.57 / TEX_PT_PER_CM;
...
function mathLegendBoxWidth(entries, fontScale, imageWidth, options = {}) {
  const hasMatrixEntry = entries.some((entry) => {
    const parsed = parseMathText(entry);
    return Boolean(parsed && hasMatrixEnvironmentTex(parsed.tex));
  });
  ...
  return (
    ...
    imageWidth +
    (hasMatrixEntry ? PGFPLOTS_LEGEND_MATRIX_TEXT_GAP : PGFPLOTS_LEGEND_TEXT_GAP) +
    textWidth +
