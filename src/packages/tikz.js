export const texPackage = {
  "name": "tikz",
  "status": "builtin",
  "implementedBy": "src/frontend/parser.js + src/engine/evaluate.js:interpretPathStatement/transformCanvasTransform/resolvedTextFontSpec/resolveAutoInlineNodePoint/autoInlineNodeAnchor/autoInlineNodeUsesOppositeAnchor/resolveSlopedInlineNodePoint/inlineNodePathTangent/flushOrthogonalInlinePathNodes + src/tex/fontSpec.js + src/tikz/textMetrics.js + src/renderers/svg/textLayout.js + src/renderers/svg/richText.js + src/renderers/svg/renderSvg.js",
  ...
  "features": [
    ...
    "cubic path-node pos with local-tangent auto/swap compass anchors and sloped rotation",
    "combined auto and sloped path nodes with local north/south anchors on line, curve, and orthogonal timer legs"
  ],
  "requires": [
    "pgf",
    "pgffor"
  ...
  "localSourceReviewedLatest": "/usr/local/texlive/2025/texmf-dist/tex/generic/pgf/frontendlayer/tikz/tikz.code.tex (/tikz/use as bounding box is an executable path mode, lines 534 and 1958; path-node pos/auto/swap/sloped options, lines 1095-1125; hv/vh construction, lines 2872-2938; online node transformation orders auto before the path timer, lines 4156-4171; auto temporarily toggles pgfslopedattime and selects the local anchor, lines 4462-4535; hv/vh timers split time evenly at .5, lines 4894-4946; cubic timer delegates to pgftransformcurveattime, lines 4948-4950); /usr/local/texlive/2025/texmf-dist/tex/generic/pgf/basiclayer/pgfcorepathusage.code.tex (/pgf/use as bounding box invokes PGF's bounding-box callback, line 41); /usr/local/texlive/2025/texmf-dist/tex/generic/pgf/basiclayer/pgfcoretransformations.code.tex (pgftransformlineattime and pgftransformcurveattime compute the point plus normalized tangent frame and upright correction, lines 468-497 and 566-590); /usr/local/texlive/2025/texmf-dist/doc/generic/pgf/pgfmanual-en-tikz-shapes.tex (path-node pos, orthogonal midpoint timing, auto, swap, sloped, and allow upside down semantics, lines 1866-2040); /Library/TeX/texbin/tikztosvg (local wrapper creates standalone[crop,tikz,multi=false] then appends fragment input)",
  "caseCount": 1077,
  "caseExamples": [
