import { roundNumber } from "../../engine/math.js";
...
function normalizeMatrixDelimiter(value) {
  ...
  if (text === "[" || text === "{[}") return "[";
  if (text === "]" || text === "{]}") return "]";
  if (text === "(" || text === "{(}" || text === "left parenthesis") return "(";
  if (text === ")" || text === "{)}" || text === "right parenthesis") return ")";
  return null;
}
...
function matrixDelimiterPath(delimiter, side, x, top, bottom, tick, style) {
  ...
  if (delimiter === "(" || delimiter === ")") {
    ...
    const quarter = Math.max(0.02, (top - bottom) * 0.24);
    return {
      type: "path",
      subtype: "matrix-delimiter",
      style,
      commands: [
        ...
        {
          type: "curveTo",
          x1: roundNumber(x + inward * tick * 0.22),
          y1: roundNumber(top),
          x2: roundNumber(x),
          y2: roundNumber(middle + quarter),
          x: roundNumber(x),
          y: roundNumber(middle)
        },
        {
          type: "curveTo",
          x1: roundNumber(x),
          y1: roundNumber(middle - quarter),
          x2: roundNumber(x + inward * tick * 0.22),
          y2: roundNumber(bottom),
          x: roundNumber(x + inward * tick),
          y: roundNumber(bottom)
        }
      ]
    };
  }
