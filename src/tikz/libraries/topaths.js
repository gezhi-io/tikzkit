...
tikzLibrary.implementedBy += "; src/engine/evaluate.js:resolveDynamicOptions for edge segments; src/engine/options.js:substituteDynamicOptionValues";
tikzLibrary.features.push("foreach-bound edge option keys resolved before to-path geometry");
tikzLibrary.localSourceReviewed += "; /usr/local/texlive/2025/texmf-dist/tex/generic/pgf/utilities/pgffor.code.tex (iteration variables are bound while each edge body is executed)";
tikzLibrary.notes += " Reviewed again on 2026-09-05 using the Bellman-Ford Beamer fixture: a slash-bound option variable whose value is `bend right` is expanded before curve-to selection, so both directed b/e edges receive independent cubic geometry and tangent-aligned arrow tips in every frame. The three selected-frame QA directories contain MacTeX, TikZKit, tikztosvg, and registered-diff artifacts. Arbitrary macro-generated comma lists and repeated identical-key timelines remain partial.";
