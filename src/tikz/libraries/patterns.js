export const tikzLibrary = {
  ...
  "notes": "Form-only patterns keep PGF's paint bounds separate from tile steps. SVG path fills use clipped, explicit tile expansion so the y-axis direction and overhanging tile seams match PGF/tikztosvg. Preamble declarations now retain simple TikZ `/.store in` values when their macro names drive tile bounds, repeat steps, path coordinates, or `\\pgfsetlinewidth`; the verified flexible-hatch case matches a 9pt step and 2pt line width. Supported primitives cover line, dots, grid, checkerboard, and bricks families; pattern transforms, mutable/inherently-colored declarations, dynamic changes to declared pattern arguments after declaration, polar paths, and in-pattern color changes remain unsupported."
};
