...
import { isForbiddenSignShape, isMagnifyingGlassShape } from "../../tikz/libraries/shapes.symbols.js";
import {
  TIKZ_MARGIN,
  TIKZ_UNIT
...
function renderItem(item, unit, options = {}, index = 0, pageOrigin = { x: 0, y: 0 }) {
  ...
  if (item.type === "nodeBox") {
    ...
    if (item.shape === "circle" || item.shape === "ellipse" || isForbiddenSignShape(item.shape) || isMagnifyingGlassShape(item.shape)) {
      const shapeClass = isForbiddenSignShape(item.shape) || isMagnifyingGlassShape(item.shape)
