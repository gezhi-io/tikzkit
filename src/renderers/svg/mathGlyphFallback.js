import { escapeAttribute } from "./escape.js";
import { formatSvgNumber as format } from "./format.js";

const TEX_PT_PER_CM = 28.4527559;
const SVG_PT_PER_CM = 72 / 2.54;
const TEX_TO_SVG_POINT_SCALE = TEX_PT_PER_CM / SVG_PT_PER_CM;

const TEX_MATH_GLYPHS = {
  "\\cdot": {
    // Computer Modern \cdot, copied from tikztosvg's TeX glyph path for a 10pt math label.
    className: "cdot",
    path:
      "M 1.90625 -2.5 C 1.90625 -2.78125 1.671875 -3.015625 1.390625 -3.015625 C 1.09375 -3.015625 0.859375 -2.78125 0.859375 -2.5 C 0.859375 -2.203125 1.09375 -1.96875 1.390625 -1.96875 C 1.671875 -1.96875 1.90625 -2.203125 1.90625 -2.5 Z M 1.90625 -2.5",
    advance: 2.78125,
    centerOffset: 1.390625,
    baselineFromCenter: 3.73,
    textBoxHeight: 1.046875
  },
  x: {
    // Computer Modern math italic x, copied from tikztosvg's TeX glyph path for a 10pt math label.
    path:
      "M 3.328125 -3.015625 C 3.390625 -3.265625 3.625 -4.1875 4.3125 -4.1875 C 4.359375 -4.1875 4.609375 -4.1875 4.8125 -4.0625 C 4.53125 -4 4.34375 -3.765625 4.34375 -3.515625 C 4.34375 -3.359375 4.453125 -3.171875 4.71875 -3.171875 C 4.9375 -3.171875 5.25 -3.34375 5.25 -3.75 C 5.25 -4.265625 4.671875 -4.40625 4.328125 -4.40625 C 3.75 -4.40625 3.40625 -3.875 3.28125 -3.65625 C 3.03125 -4.3125 2.5 -4.40625 2.203125 -4.40625 C 1.171875 -4.40625 0.59375 -3.125 0.59375 -2.875 C 0.59375 -2.765625 0.703125 -2.765625 0.71875 -2.765625 C 0.796875 -2.765625 0.828125 -2.796875 0.84375 -2.875 C 1.1875 -3.9375 1.84375 -4.1875 2.1875 -4.1875 C 2.375 -4.1875 2.71875 -4.09375 2.71875 -3.515625 C 2.71875 -3.203125 2.546875 -2.546875 2.1875 -1.140625 C 2.03125 -0.53125 1.671875 -0.109375 1.234375 -0.109375 C 1.171875 -0.109375 0.953125 -0.109375 0.734375 -0.234375 C 0.984375 -0.296875 1.203125 -0.5 1.203125 -0.78125 C 1.203125 -1.046875 0.984375 -1.125 0.84375 -1.125 C 0.53125 -1.125 0.296875 -0.875 0.296875 -0.546875 C 0.296875 -0.09375 0.78125 0.109375 1.21875 0.109375 C 1.890625 0.109375 2.25 -0.59375 2.265625 -0.640625 C 2.390625 -0.28125 2.75 0.109375 3.34375 0.109375 C 4.375 0.109375 4.9375 -1.171875 4.9375 -1.421875 C 4.9375 -1.53125 4.859375 -1.53125 4.828125 -1.53125 C 4.734375 -1.53125 4.71875 -1.484375 4.6875 -1.421875 C 4.359375 -0.34375 3.6875 -0.109375 3.375 -0.109375 C 2.984375 -0.109375 2.828125 -0.421875 2.828125 -0.765625 C 2.828125 -0.984375 2.875 -1.203125 2.984375 -1.640625 Z M 3.328125 -3.015625",
    advance: 5.7,
    centerOffset: 2.85,
    baselineFromCenter: 3.73,
    textBoxHeight: 7.83
  },
  y: {
    path:
      "M 4.84375 -3.796875 C 4.890625 -3.9375 4.890625 -3.953125 4.890625 -4.03125 C 4.890625 -4.203125 4.75 -4.296875 4.59375 -4.296875 C 4.5 -4.296875 4.34375 -4.234375 4.25 -4.09375 C 4.234375 -4.03125 4.140625 -3.734375 4.109375 -3.546875 C 4.03125 -3.296875 3.96875 -3.015625 3.90625 -2.75 L 3.453125 -0.953125 C 3.421875 -0.8125 2.984375 -0.109375 2.328125 -0.109375 C 1.828125 -0.109375 1.71875 -0.546875 1.71875 -0.921875 C 1.71875 -1.375 1.890625 -2 2.21875 -2.875 C 2.375 -3.28125 2.421875 -3.390625 2.421875 -3.59375 C 2.421875 -4.03125 2.109375 -4.40625 1.609375 -4.40625 C 0.65625 -4.40625 0.296875 -2.953125 0.296875 -2.875 C 0.296875 -2.765625 0.390625 -2.765625 0.40625 -2.765625 C 0.515625 -2.765625 0.515625 -2.796875 0.5625 -2.953125 C 0.84375 -3.890625 1.234375 -4.1875 1.578125 -4.1875 C 1.65625 -4.1875 1.828125 -4.1875 1.828125 -3.875 C 1.828125 -3.625 1.71875 -3.359375 1.65625 -3.171875 C 1.25 -2.109375 1.078125 -1.546875 1.078125 -1.078125 C 1.078125 -0.1875 1.703125 0.109375 2.296875 0.109375 C 2.6875 0.109375 3.015625 -0.0625 3.296875 -0.34375 C 3.171875 0.171875 3.046875 0.671875 2.65625 1.203125 C 2.390625 1.53125 2.015625 1.828125 1.5625 1.828125 C 1.421875 1.828125 0.96875 1.796875 0.796875 1.40625 C 0.953125 1.40625 1.09375 1.40625 1.21875 1.28125 C 1.328125 1.203125 1.421875 1.0625 1.421875 0.875 C 1.421875 0.5625 1.15625 0.53125 1.0625 0.53125 C 0.828125 0.53125 0.5 0.6875 0.5 1.171875 C 0.5 1.671875 0.9375 2.046875 1.5625 2.046875 C 2.578125 2.046875 3.609375 1.140625 3.890625 0.015625 Z M 4.84375 -3.796875",
    advance: 5.2,
    centerOffset: 2.6,
    baselineFromCenter: 3.73,
    textBoxHeight: 9.9
  },
  two: {
    path:
      "M 3.515625 -1.265625 L 3.28125 -1.265625 C 3.265625 -1.109375 3.1875 -0.703125 3.09375 -0.640625 C 3.046875 -0.59375 2.515625 -0.59375 2.40625 -0.59375 L 1.125 -0.59375 C 1.859375 -1.234375 2.109375 -1.4375 2.515625 -1.765625 C 3.03125 -2.171875 3.515625 -2.609375 3.515625 -3.265625 C 3.515625 -4.109375 2.78125 -4.625 1.890625 -4.625 C 1.03125 -4.625 0.4375 -4.015625 0.4375 -3.375 C 0.4375 -3.03125 0.734375 -2.984375 0.8125 -2.984375 C 0.96875 -2.984375 1.171875 -3.109375 1.171875 -3.359375 C 1.171875 -3.484375 1.125 -3.734375 0.765625 -3.734375 C 0.984375 -4.21875 1.453125 -4.375 1.78125 -4.375 C 2.484375 -4.375 2.84375 -3.828125 2.84375 -3.265625 C 2.84375 -2.65625 2.40625 -2.1875 2.1875 -1.9375 L 0.515625 -0.265625 C 0.4375 -0.203125 0.4375 -0.1875 0.4375 0 L 3.3125 0 Z M 3.515625 -1.265625"
  },
  plus: {
    path:
      "M 4.078125 -2.296875 L 6.859375 -2.296875 C 7 -2.296875 7.1875 -2.296875 7.1875 -2.5 C 7.1875 -2.6875 7 -2.6875 6.859375 -2.6875 L 4.078125 -2.6875 L 4.078125 -5.484375 C 4.078125 -5.625 4.078125 -5.8125 3.875 -5.8125 C 3.671875 -5.8125 3.671875 -5.625 3.671875 -5.484375 L 3.671875 -2.6875 L 0.890625 -2.6875 C 0.75 -2.6875 0.5625 -2.6875 0.5625 -2.5 C 0.5625 -2.296875 0.75 -2.296875 0.890625 -2.296875 L 3.671875 -2.296875 L 3.671875 0.5 C 3.671875 0.640625 3.671875 0.828125 3.875 0.828125 C 4.078125 0.828125 4.078125 0.640625 4.078125 0.5 Z M 4.078125 -2.296875"
  },
  equals: {
    path:
      "M 6.84375 -3.265625 C 7 -3.265625 7.1875 -3.265625 7.1875 -3.453125 C 7.1875 -3.65625 7 -3.65625 6.859375 -3.65625 L 0.890625 -3.65625 C 0.75 -3.65625 0.5625 -3.65625 0.5625 -3.453125 C 0.5625 -3.265625 0.75 -3.265625 0.890625 -3.265625 Z M 6.859375 -1.328125 C 7 -1.328125 7.1875 -1.328125 7.1875 -1.53125 C 7.1875 -1.71875 7 -1.71875 6.84375 -1.71875 L 0.890625 -1.71875 C 0.75 -1.71875 0.5625 -1.71875 0.5625 -1.53125 C 0.5625 -1.328125 0.75 -1.328125 0.890625 -1.328125 Z M 6.859375 -1.328125"
  },
  one: {
    path:
      "M 2.9375 -6.375 C 2.9375 -6.625 2.9375 -6.640625 2.703125 -6.640625 C 2.078125 -6 1.203125 -6 0.890625 -6 L 0.890625 -5.6875 C 1.09375 -5.6875 1.671875 -5.6875 2.1875 -5.953125 L 2.1875 -0.78125 C 2.1875 -0.421875 2.15625 -0.3125 1.265625 -0.3125 L 0.953125 -0.3125 L 0.953125 0 C 1.296875 -0.03125 2.15625 -0.03125 2.5625 -0.03125 C 2.953125 -0.03125 3.828125 -0.03125 4.171875 0 L 4.171875 -0.3125 L 3.859375 -0.3125 C 2.953125 -0.3125 2.9375 -0.421875 2.9375 -0.78125 Z M 2.9375 -6.375"
  }
};

const SIMPLE_FORMULA_LAYOUTS = {
  "x^2+y^2=1": {
    width: 50.319,
    centerX: 25.158,
    entries: [
      { key: "x", x: 0, y: 3.086 },
      { key: "two", x: 5.694, y: -0.529 },
      { key: "plus", x: 12.377, y: 3.086 },
      { key: "y", x: 22.34, y: 3.086 },
      { key: "two", x: 27.582, y: -0.529 },
      { key: "equals", x: 34.818, y: 3.086 },
      { key: "one", x: 45.31858, y: 3.086 }
    ]
  }
};

export function simpleMathGlyphFallback(tex) {
  const key = String(tex || "").trim();
  const glyph = TEX_MATH_GLYPHS[key];
  return glyph ? { key, ...glyph } : null;
}

export function simpleMathGlyphFormulaFallback(tex) {
  const key = String(tex || "").replace(/\s+/g, "");
  const layout = SIMPLE_FORMULA_LAYOUTS[key];
  return layout ? { key, ...layout } : null;
}

export function renderSimpleMathGlyphFallback(item, glyph, fontSize, unit, color) {
  const x = item.x * unit;
  const y = -item.y * unit;
  const scale = glyphScale(fontSize);
  const anchor = item.style?.textAnchor || "middle";
  const anchorOffset = anchor === "end" ? glyph.advance : anchor === "start" ? 0 : glyph.centerOffset;
  const originX = x - anchorOffset * scale;
  const baselineY = y + glyph.baselineFromCenter * scale;
  return `<path class="tikz-math-glyph tikz-math-glyph-${escapeAttribute(glyph.className || glyph.key)}" d="${glyph.path}" fill="${escapeAttribute(
    color
  )}" transform="translate(${format(originX)} ${format(baselineY)}) scale(${format(scale)})" />`;
}

export function simpleMathGlyphRenderBox(glyph, fallbackBox) {
  if (!glyph || !Number.isFinite(glyph.textBoxHeight)) return null;
  const fontSize = Number(fallbackBox?.fontSize);
  if (!Number.isFinite(fontSize) || fontSize <= 0) return null;
  const scale = fontSize / 10;
  return {
    width: Number(fallbackBox?.width) || (Number(glyph.advance) || 0) * scale,
    height: glyph.textBoxHeight * scale
  };
}

export function renderSimpleMathGlyphFormulaFallback(item, formula, fontSize, unit, color) {
  const x = item.x * unit;
  const y = -item.y * unit;
  const scale = glyphScale(fontSize);
  const anchor = item.style?.textAnchor || "middle";
  const anchorOffset = anchor === "end" ? formula.width : anchor === "start" ? 0 : formula.centerX;
  const paths = formula.entries
    .map((entry) => {
      const glyph = TEX_MATH_GLYPHS[entry.key];
      if (!glyph) return "";
      const originX = x + (entry.x - anchorOffset) * scale;
      const originY = y + entry.y * scale;
      return `<path class="tikz-math-glyph tikz-math-glyph-${escapeAttribute(entry.key)}" d="${glyph.path}" fill="${escapeAttribute(
        color
      )}" transform="translate(${format(originX)} ${format(originY)}) scale(${format(scale)})" />`;
    })
    .join("");
  return `<g class="tikz-math-glyphs tikz-math-glyphs-formula" data-tex="${escapeAttribute(formula.key)}">${paths}</g>`;
}

function glyphScale(fontSize) {
  // The glyph paths are copied from tikztosvg's final SVG, whose coordinates are SVG pt.
  // TikZKit's font size is derived from TeX pt; compensate before placing raw glyph paths.
  return (fontSize / 10) * TEX_TO_SVG_POINT_SCALE;
}
