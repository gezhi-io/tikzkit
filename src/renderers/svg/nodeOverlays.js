...
export function renderNodeBoxShadow(item, shadow, unit) {
  ...
  if (shadowItem.shape === "circle" || shadowItem.shape === "circleSplit" || shadowItem.shape === "ellipse" || shadowItem.shape === "ellipseSplit") {
...
export function renderDoubleNodeOutline(item, unit) {
  ...
  const halfWidth = (item.width * unit) / 2;
  const halfHeight = (item.height * unit) / 2;
  if (item.shape === "circle" || item.shape === "circleSplit" || item.shape === "ellipse" || item.shape === "ellipseSplit") {
