...
\end{tikzpicture}`;
  ...
  assert.match(svg, /<radialGradient[^>]+id="tikzkit-[\da-f]{16}-tikz-fading-gradient-circle-with-fuzzy-edge-15-percent-radial"/);
  ...
  assert.match(svg, /<radialGradient[^>]+id="tikzkit-[\da-f]{16}-tikz-fading-gradient-fuzzy-ring-15-percent-radial"/);
  assert.match(svg, /<stop offset="46\.25%" stop-color="white"/);
  assert.match(svg, /mask="url\(#tikzkit-[\da-f]{16}-tikz-fading-circle-with-fuzzy-edge-15-percent-mask\)"/);
  assert.match(svg, /mask="url\(#tikzkit-[\da-f]{16}-tikz-fading-fuzzy-ring-15-percent-mask\)"/);
  assert.match(svg, /mask="url\(#tikzkit-[\da-f]{16}-tikz-fading-circle-with-fuzzy-edge-20-percent-mask\)"/);
});
