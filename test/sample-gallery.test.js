...
test("web sample gallery provides 108 real TikZ source blocks", () => {
  ...
  assert.equal(tikzParts.length, 108);
  assert.equal(REAL_GALLERY_CASES.length, 108);
  assert.equal(petarVCases.length, 65);
  assert.match(source, /Case 001/);
  assert.match(source, /Case 100/);
  assert.match(source, /Case 108/);
  assert.match(source, /PetarV-\/TikZ/);
  assert.match(source, /Packt GitHub|TikZ\.net|MacTeX tikz-3dplot|MacTeX tikz-bagua/);
