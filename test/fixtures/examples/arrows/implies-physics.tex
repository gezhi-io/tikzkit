...
\begin{tikzpicture}[
  ...
  \draw[blue,double=cyan!15,line width=1pt,double distance=.65pt,-implies]
    (force) -- node[above] {$F=ma$} (accel);
  \draw[green!50!black,double=white,line width=.8pt,double distance=.5pt,-implies]
    (accel) -- node[above] {$\int\!a\,dt$} (motion);
  \draw[orange!85!black,double=yellow!25,line width=1.2pt,double distance=.8pt,-implies]
    (motion) to[bend left=16] node[right,sloped] {measure} (sensor);
  \draw[purple,double=white,line width=.8pt,double distance=.5pt,implies-]
    (accel) -- node[right] {feedback} (sensor);
  \draw[red,double=white,line width=.8pt,double distance=.5pt,-implies]
