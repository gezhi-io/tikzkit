...
\begin{tikzpicture}[
  ...
  \draw[map,blue,-{serif cm}] (V) -- node[above] {$f$} (W);
  \draw[map,purple,-{spaced serif cm}] (V) -- node[left] {$\pi_V$} (Q);
  \draw[map,orange!85!black,-{serif cm}] (W) -- node[right] {$\pi_W$} (R);
  \draw[map,green!50!black,-{spaced serif cm}] (Q) -- node[below] {$\bar f$} (R);
  \draw[map,dashed,red,{spaced serif cm}-{serif cm}]
